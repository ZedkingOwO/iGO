...
// %d解释为整数，%c解释为字符。内存中一个数据，看你赋子它什么类型，就可以看到不同的东西号
...
```go
	...
	fmt.Printf("%f\n", 100+2.65)   // 默认 .000000
	fmt.Printf("%.2f\n", 100+2.65) // %.2f 小数点后两位
	fmt.Printf("%30f\n", 100+2.65) // %30f 宽度右对齐，30包括打印打字符多余的用空格补充
	fmt.Printf("%-30f\n", 100+2.65)// %-30f宽度左对齐
...
```bash
#二进制和十六进制之间的转换基于它们基数之间的关系：十六进制是基于16的数制，而二进制是基于2的数制。由于16是2的4次幂，所以每4位二进制数可以表示一个十六进制数位。八进制同理
2^4 =16
二进制    十六进制
0000        0
0001        1
...
\' 单引号是用来界定 a是字符   让'代表原来的意思\'
\" 双引号是用来界定 a是字符   让'代表原来的意思\'
```
...
单引号用来表示字符，字面量表达，本质上是int 32 或 byte(uint8)
...
整数
%b 二进制； %o 八进制；%O 代表有权重；%x 十六进制小写；%X16 进制大写
%U 
